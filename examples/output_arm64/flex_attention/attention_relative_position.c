...
// FUSED LOOP (4 ops): Q=TLOAD(Q_mem,0,0); K=TLOAD(K_mem,0,0); V=TLOAD(V_mem,0,0); rel_pos_bias=TLOAD(bias_mem,0,0)
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _vl3 = vld1q_f32(&bias_mem[_row * 8 + _col]);
        vst1q_f32(&rel_pos_bias[_row][_col], _vl3);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
// FUSED LOOP (2 ops): scaled=TMULS(scores,0.35355339059327373f); biased_scores=TADD(scaled,rel_pos_bias)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _v8 = vld1q_f32(&rel_pos_bias[_row][_col]);
