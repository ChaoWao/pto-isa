...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        vst1q_f32(&causal_mask[_row][_col], _vl3);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
// FUSED LOOP (2 ops): scaled=TMULS(scores,0.35355339059327373f); masked_scores=TADD(scaled,causal_mask)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _v8 = vld1q_f32(&causal_mask[_row][_col]);
