...
// FUSED LOOP (4 ops): Q=TLOAD(Q_mem,0,0); K=TLOAD(K_mem,0,0); V=TLOAD(V_mem,0,0); prefix_mask=TLOAD(mask_mem,0,0)
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        vst1q_f32(&prefix_mask[_row][_col], _vl3);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
// FUSED LOOP (2 ops): scaled=TMULS(scores,0.35355339059327373f); masked_scores=TADD(scaled,prefix_mask)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _v8 = vld1q_f32(&prefix_mask[_row][_col]);
