...
float scores[8][8];
float scaled[8][8];
float score_mod_bias[8][8];
float modified_scores[8][8];
float row_sum[8][1];
float shifted[8][8];
...
// FUSED LOOP (4 ops): Q=TLOAD(Q_mem,0,0); K=TLOAD(K_mem,0,0); V=TLOAD(V_mem,0,0); score_mod_bias=TLOAD(score_mod_mem,0,0)
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _vl3 = vld1q_f32(&score_mod_mem[_row * 8 + _col]);
        vst1q_f32(&score_mod_bias[_row][_col], _vl3);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
// FUSED LOOP (2 ops): scaled=TMULS(scores,0.35355339059327373f); modified_scores=TADD(scaled,score_mod_bias)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _v8 = vld1q_f32(&score_mod_bias[_row][_col]);
        float32x4_t _vr9 = vaddq_f32(_v7, _v8);
        vst1q_f32(&modified_scores[_row][_col], _vr9);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (int _col = 0; _col < 8; _col++) {
        _sum += modified_scores[_row][_col];
    }
    row_sum[_row][0] = _sum;}
...
// TROWEXPANDSUB: shifted = modified_scores - broadcast(row_sum)
for (int _row = 0; _row < 8; _row++) {
    float _broadcast_val = row_sum[_row][0];
    for (int _col = 0; _col < 8; _col++) {
        shifted[_row][_col] = modified_scores[_row][_col] - _broadcast_val;
    }}
