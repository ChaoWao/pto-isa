...
// TROWEXPANDSUB: shifted = capped_scores - broadcast(row_sum)
for (int _row = 0; _row < 8; _row++) {
    float _broadcast_val = row_sum[_row][0];
    for (int _col = 0; _col < 8; _col++) {
        shifted[_row][_col] = capped_scores[_row][_col] - _broadcast_val;
    }}
...
for (int _row = 0; _row < 8; _row++) {
    ...
    // Vectorized loop
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        float32x4_t _v28 = vld1q_f32(&shifted[_row][_col]);
        float32x4_t _vr29 = _v28;
        vst1q_f32(&exp_scores[_row][_col], _vr29);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
...
for (int _row = 0; _row < 8; _row++) {
    ...
    // Vectorized loop
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        float32x4_t _vs30 = vld1q_f32(&output[_row][_col]);
        vst1q_f32(&output_mem[_row * 8 + _col], _vs30);
    }
    // Scalar cleanup
    for (; _col < 8; _col++) {
