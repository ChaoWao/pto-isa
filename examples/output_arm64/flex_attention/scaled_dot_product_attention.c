...
// FUSED LOOP (1 ops): scaled_scores=TMULS(scores,0.35355339059327373f)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 1; _col += 4) {
        float32x4_t _v7 = vld1q_f32(&row_max[_row][_col]);
        float32x4_t _vr8 = vdivq_f32(_v7, _vs6);
        vst1q_f32(&row_max[_row][_col], _vr8);
    }
    // Scalar cleanup
    for (; _col < 1; _col++) {
...
// TROWEXPANDSUB: shifted = scaled_scores - broadcast(row_max)
for (int _row = 0; _row < 8; _row++) {
    float _broadcast_val = row_max[_row][0];
    for (int _col = 0; _col < 8; _col++) {
        shifted[_row][_col] = scaled_scores[_row][_col] - _broadcast_val;
    }}
...
// TROWEXPANDDIV: attention_weights = exp_scores / broadcast(row_sum)
for (int _row = 0; _row < 8; _row++) {
    float _broadcast_val = row_sum[_row][0];
    for (int _col = 0; _col < 8; _col++) {
        attention_weights[_row][_col] = exp_scores[_row][_col] / _broadcast_val;
    }}
...
for (int _i = 0; _i < 8; _i++) {
    for (int _j = 0; _j < 8; _j++) {
        float _sum = 0.0f;
        for (int _k = 0; _k < 8; _k++) {
            _sum += attention_weights[_i][_k] * V[_k][_j];}
        output[_i][_j] = _sum;}}
