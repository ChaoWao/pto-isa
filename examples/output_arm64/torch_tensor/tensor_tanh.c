...
// FUSED LOOP (7 ops): x=TLOAD(input,0,0); x2=TMULS(x,2.0f); exp_2x=TEXP(x2); numerator=TADDS(exp_2x,-1.0f); denominator=TADDS(exp_2x,1.0f); result=TDIV(numerator,denominator); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (; _col < 8; _col++) {
        ...
        exp_2x[_row][_col] = expf(x2[_row][_col]);
