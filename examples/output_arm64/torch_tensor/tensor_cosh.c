...
// FUSED LOOP (7 ops): x=TLOAD(input,0,0); neg_x=TNEG(x); exp_x=TEXP(x); exp_neg_x=TEXP(neg_x); sum_val=TADD(exp_x,exp_neg_x); result=TDIVS(sum_val,2.0f); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        vst1q_f32(&sum_val[_row][_col], _vr10);
        float32x4_t _v11 = vld1q_f32(&sum_val[_row][_col]);
    ...
    for (; _col < 8; _col++) {
        ...
        sum_val[_row][_col] = exp_x[_row][_col] + exp_neg_x[_row][_col];
        result[_row][_col] = sum_val[_row][_col] / 2.0f;
