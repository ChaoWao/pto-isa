...
// FUSED LOOP (7 ops): x=TLOAD(input,0,0); neg_x=TNEG(x); exp_neg_x=TEXP(neg_x); one_plus=TADDS(exp_neg_x,1.0f); softplus=TLOG(one_plus); result=TNEG(softplus); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (; _col < 8; _col++) {
        ...
        softplus[_row][_col] = logf(one_plus[_row][_col]);
