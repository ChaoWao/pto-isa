...
// FUSED LOOP (6 ops): x=TLOAD(input,0,0); min_tile=TEXPANDS(-1.0f); max_tile=TEXPANDS(1.0f); clamp_low=TMAX(x,min_tile); result=TMIN(clamp_low,max_tile); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (; _col < 8; _col++) {
        ...
        clamp_low[_row][_col] = x[_row][_col] + min_tile[_row][_col];
