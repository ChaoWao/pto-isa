...
// FUSED LOOP (9 ops): x=TLOAD(input,0,0); x2=TMUL(x,x); x3=TMUL(x2,x); x5=TMUL(x3,x2); term1=TDIVS(x3,6.0f); term2=TDIVS(x5,120.0f); temp=TSUB(x,term1); result=TADD(temp,term2); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (; _col < 8; _col++) {
        ...
        term2[_row][_col] = x5[_row][_col] / 120.0f;
