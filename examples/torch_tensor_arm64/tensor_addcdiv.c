...
// FUSED LOOP (7 ops): self=TLOAD(input_self,0,0); tensor1=TLOAD(input_t1,0,0); tensor2=TLOAD(input_t2,0,0); quot=TDIV(tensor1,tensor2); scaled=TMULS(quot,1.0f); result=TADD(self,scaled); output=TSTORE(result,0,0)
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 8; _col += 4) {
        ...
        float32x4_t _vr6 = vdivq_f32(_v4, _v5);
        vst1q_f32(&quot[_row][_col], _vr6);
        float32x4_t _v7 = vld1q_f32(&quot[_row][_col]);
