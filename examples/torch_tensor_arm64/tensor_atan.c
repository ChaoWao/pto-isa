...
// FUSED LOOP (9 ops): x=TLOAD(input,0,0); x2=TMUL(x,x); x3=TMUL(x2,x); x5=TMUL(x3,x2); term1=TDIVS(x3,3.0f); term2=TDIVS(x5,5.0f); temp=TSUB(x,term1); result=TADD(temp,term2); output=TSTORE(result,0,0)
float32x4_t _vs0 = vdupq_n_f32(3.0f);
float32x4_t _vs1 = vdupq_n_f32(5.0f);
