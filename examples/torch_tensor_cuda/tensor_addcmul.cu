...
__device__ float prod[8][8];
__device__ float scaled[8][8];
__device__ float result[8][8];

__global__ void tensor_addcmul_kernel() {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 6 loop overheads saved

    // FUSED (7 ops): self=TLOAD(...); tensor1=TLOAD(...); tensor2=TLOAD(...); prod=TMUL(...); scaled=TMULS(...); result=TADD(...); output=TSTORE(...)
    if (_row < 8 && _col < 8) {
...
void tensor_addcmul() {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tensor_addcmul_kernel<<<grid, block>>>();
