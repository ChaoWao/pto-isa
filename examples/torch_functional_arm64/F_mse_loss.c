...
// FUSED LOOP (4 ops): pred=TLOAD(input,0,0); target=TLOAD(target_mem,0,0); diff=TSUB(pred,target); sq_diff=TMUL(diff,diff)
...
for (int _row = 0; _row < 1; _row++) {
    ...
    for (_col = 0; _col + 4 <= 1; _col += 4) {
        float32x4_t _v9 = vld1q_f32(&result[_row][_col]);
