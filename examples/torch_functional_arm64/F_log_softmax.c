...
// TROWEXPANDSUB: x_shifted = x - broadcast(row_mean)
for (int _row = 0; _row < 8; _row++) {
...
for (int _row = 0; _row < 8; _row++) {
    ...
    for (_col = 0; _col + 4 <= 1; _col += 4) {
        ...
        vst1q_f32(&log_sum[_row][_col], _vr7);
    }
    // Scalar cleanup
    for (; _col < 1; _col++) {
...
// TROWEXPANDSUB: result = x_shifted - broadcast(log_sum)
for (int _row = 0; _row < 8; _row++) {
    float _broadcast_val = log_sum[_row][0];
    for (int _col = 0; _col < 8; _col++) {
        result[_row][_col] = x_shifted[_row][_col] - _broadcast_val;
    }}
