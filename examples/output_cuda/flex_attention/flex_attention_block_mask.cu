...
__device__ float block_mask[8][8];
...
__global__ void flex_attention_block_mask_kernel() {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 4 loop overheads saved

    // FUSED (4 ops): Q=TLOAD(...); K=TLOAD(...); V=TLOAD(...); block_mask=TLOAD(...)
    if (_row < 8 && _col < 8) {
...
void flex_attention_block_mask() {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    flex_attention_block_mask_kernel<<<grid, block>>>();
