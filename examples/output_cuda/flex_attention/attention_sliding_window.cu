...
__device__ float window_mask[8][8];
...
__global__ void attention_sliding_window_kernel() {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 4 loop overheads saved

    // FUSED (4 ops): Q=TLOAD(...); K=TLOAD(...); V=TLOAD(...); window_mask=TLOAD(...)
    if (_row < 8 && _col < 8) {
...
void attention_sliding_window() {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_sliding_window_kernel<<<grid, block>>>();
