...
private:
  ...
  // Device orchestration: GM pointer to PTO2 shared memory (device)
  void* pto2_gm_sm_ptr_;
  int32_t pto2_sm_size_;   // Size in bytes
  int orch_arg_count_;
...
public:
    ...
    /** PTO2 shared memory size in bytes (set when allocating; 0 if not set). */
