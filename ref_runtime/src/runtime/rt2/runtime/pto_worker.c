...
int32_t pto2_worker_get_task(PTO2WorkerContext* worker) {
    ...
    while (!worker->shutdown) {
        ...
        /* Timed wait could use timeout; currently using indefinite wait */
        ...
        while (!worker->shutdown) {
            if (pto2_ready_queue_empty(queue)) {
                ...
                int64_t my_clock = PTO2_LOAD_ACQUIRE(&ctx->worker_current_cycle[worker->worker_id]);
                (void)my_clock;  /* reserved for min-clock tie-breaking */
...
void pto2_worker_execute_task(PTO2WorkerContext* worker, int32_t task_id) {
    ...
    if (task->func_ptr) {
        ...
        for (int i = 0; i < task->num_outputs; i++) {
            ...
            int32_t slot_size = (i + 1 < task->num_outputs)
                ? (task->output_offsets[i + 1] - task->output_offsets[i])
                : (int32_t)((char*)task->packed_buffer_end - (char*)task->packed_buffer_base)
                  - task->output_offsets[i];
            fprintf(stderr, "[PTO2 worker] task_id=%d output[%d] ptr=%p slot_size=%d\n",
                    task_id, i, out_ptr, slot_size);
            args[num_args++] = out_ptr;
        }
